/** OpenSquilla Web UI — Theme manager (dark / light toggle). */

const Theme = (() => {
  const STORAGE_KEY = 'opensquilla-theme';
  ...
  function _updateToggle() {
    ...
    const current = I18n.t('theme.current', { mode: _current });
    el.title = current;
    el.setAttribute('aria-label', current + '. ' + I18n.t('theme.toggle'));
